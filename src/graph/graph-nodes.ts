...
export default class GraphNodes {
  ...
  public updateNodes(nodes: INode[]): void {
    const nodesFunc = function(
      this: SVGGElement,
      _datum: undefined,                              // tslint:disable-line:variable-name
      _index: number,                                 // tslint:disable-line:variable-name
      _groups: SVGGElement[] | ArrayLike<SVGGElement> // tslint:disable-line:variable-name
    ) {
      return nodes;
    };

    // tslint:disable-next-line:variable-name
    const idFunc = function(
      this: SVGGElement,
      datum: INode,
      _index: number,                                 // tslint:disable-line:variable-name
      _groups: SVGGElement[] | ArrayLike<SVGGElement> // tslint:disable-line:variable-name
  ...
  private translate = (d: INode): string => {
    return `translate(${(d.x || 0) - this.nodeWidth / 2},${(d.y || 0) -
      this.nodeHeight / 2})`;
  };
  ...
  private title = (selection: SelectionType): void => {
    selection.append("title").text((d: INode) => d.name);
  };
