...
export default class GraphModelStore {
    ...
    public runQuery(queryStatement: string, callback: (ID3Graph: GV.ID3Graph) => void): void {
        const query = {
            statements : [ {
        ...
        d3request.request(this.endpoint)
            .header("Accept", "application/json; charset=UTF-8")
            .header("Content-Type", "application/json")
            .mimeType("application/json")
            .on("error", (error) => { alert(error); })
            .on("load", (xhr: XMLHttpRequest): void => {
        ...
        for (const result of data.results) {
            for (const datum of result.data) {
                for (const node of datum.graph.nodes) {
                    ...
                    const dupes: GV.INode[] = graph.nodes.filter((n: GV.INode) => n.id === d3node.id);
                    if (dupes.length === 0) {
                ...
                for (const link of datum.graph.relationships) {
                    ...
                    const dupes: GV.ILink[] = graph.links.filter((l: GV.ILink) => l.id === d3link.id);
                    if (dupes.length === 0) {
