 ...
 */
export type SelectionType = d3selection.Selection<
  SVGGElement,
  INode,
  d3selection.BaseType,
  undefined
>;
export type ParentGroupType = d3selection.Selection<
  SVGGElement,
  undefined,
  d3selection.BaseType,
  undefined
...
export default class GraphVisualization {
  ...
  constructor(svgEl: SVGSVGElement) {
    ...
    this.graphLinks = new GraphLinks(
      this.transformSelection(),
      this.nodeWidth,
      this.nodeHeight
    );
  ...
  public showGraph(graph: ID3Graph, clearFirst = true): boolean {
    ...
    d3force
      ...
      .force(
        "link",
        d3force
          .forceLink<INode, ILink>(graph.links)
