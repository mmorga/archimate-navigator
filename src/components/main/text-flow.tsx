...
export default class TextFlow extends React.PureComponent<IProps, IState> {
  ...
  private lines(): ILine[] {
    ...
    if (width <= maxLineWidth) {
      return [{text: this.props.text, maxWidth: maxLineWidth, calcWidth: width}];
    }
    ...
    if (words.length === 1) {
      ...
      while (words.length > 0) {
        ...
        lines.push({text: line, maxWidth: maxLineWidth, calcWidth: calcLineWidth});
        line = "";
      }
      return lines;
    }
  ...
  private tspans() {
    ...
    return (
      this
        .state
        .lines
        .map(line => (<tspan x={this.lineX(idx)} dy="1.1em" className="entity-name" style={this.props.style} key={++idx}>{line.text}</tspan>))
    );
  }
}
